            ...
            Column(modifier = Modifier.weight(1f)) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    if (important) {
                        Text(
                            text = "★",
                            style = TnaTypography.Body.copy(color = TnaColors.Accent, fontWeight = FontWeight.Medium)
                        )
                        Spacer(modifier = Modifier.width(6.dp))
                    }
                    Text(text = title, style = TnaTypography.Body.copy(fontWeight = FontWeight.Medium))
                }
                Text(text = note, modifier = Modifier.padding(top = 6.dp), style = TnaTypography.AiVoice)
            }
            Text(
                text = time,
                modifier = Modifier
