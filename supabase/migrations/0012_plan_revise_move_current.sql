-- v1.2: allow moved on planned tasks even when actual_start is set (执行屏「变当前」≠ 真正开工).
-- Reschedule clears actual_start so the task is no longer treated as actively current.
