...
Deno.serve(async (req) => {
  ...
  const parsed = parseReflectionProbeRequest(body);
  ...
  try {
    ...
    const response = await buildReflectionProbe(
      supabase,
      userData.user.id,
      parsed.date,
      parsed.today_tasks,
    ...
    console.log(`[reflection/probe] ok user=${userData.user.id} date=${parsed.date}`);
    ...
    console.error("[reflection/probe] error", message);
